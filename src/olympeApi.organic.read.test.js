...
describe("OlympeApi Organic Read Tests (Real API - GET)", () => {
    ...
    describe("Teams", () => {
            ...
            if (result.length > 0) {
                ...
                for (const team of result) {
                    if (team.members && team.members.length > 0) {
                        ...
                        if (user) {
                            if (user.id) {
                                dynamicIds.userId = user.id;
                                console.log("Fetched User ID from Team:", dynamicIds.userId);
                            }
                            if (user.username) {
                                dynamicIds.username = user.username;
                                console.log("Fetched Username from Team:", dynamicIds.username);
                            }
                            if (dynamicIds.userId && dynamicIds.username) break;
                        }
                    }
                }
            }
        ...
        test("get team by id", async (ctx) => {
            ...
            // Try to extract user from detailed team view
            if (result.members && result.members.length > 0) {
                const user = result.members[0].user;
                if (user) {
                    if (user.id) dynamicIds.userId = user.id;
                    if (user.username) dynamicIds.username = user.username;
                    console.log("Fetched User from Team Detail:", dynamicIds.userId, dynamicIds.username);
                }
            }
        });
    ...
    describe("Users", () => {
        ...
        test("search user (me)", async () => {
            // Search using username or ID from extracted team member
            const searchKey = dynamicIds.username || dynamicIds.userId;
            console.log("Searching user with key:", searchKey);

            try {
                const result = await api.users.search(searchKey);
                expect(result).toBeDefined();
            } catch (err) {
                if (err.response && err.response.status === 403) {
        ...
        test("list invitations to join team", async (ctx) => {
            try {
                ...
                if (err.response && err.response.status === 403) {
                    console.warn("User invitations skipped (forbidden):", err.data?.message);
                    return;
                }
                logError(err);
            }
